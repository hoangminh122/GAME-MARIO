...
CMario *CMario::GetInstance(float x, float y,int scene_id)
{
	if (__instance == NULL) __instance = new CMario(x,y, scene_id);
	return __instance;
}

CMario::CMario(float x, float y,int _sence) : CGameObject()
{
	sence = _sence;
	numCardImage = 0;
	goBottom = false;
